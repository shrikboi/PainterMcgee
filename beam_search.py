...
IMAGE_NAME = 'FELV-cat'
directory = f'./beam_search/{IMAGE_NAME}/{NUM_OF_BRUSHSTROKES}_{RANDOM_TILES_PER_ROUND}_{BEAM_WIDTH}_{ITER_NUM}_' \
            f'{MAX_SIZE}_{MULTIPLY_WEIGHTS}_{SSIM_WEIGHT}_{COLOR_ME_TENDERS}_{LOSS_TYPE}'


class Node(Painting):
    def __init__(self, rectangle_list, target_img, weights_matrix, loss_type=LOSS_TYPE, ssim_weight=SSIM_WEIGHT,
                 parent=None):
        super().__init__(rectangle_list, target_img, weights_matrix, loss_type, ssim_weight)
        self.parent = parent
...
def random_init_canvas(target_img) -> Node:
    """
    creates a canvas that was randomly initialized
    """
    rectangle_list = random_rectangle_set(number_of_rectangles=NUM_OF_BRUSHSTROKES, target_image=target_img,
    ...
    return Node(rectangle_list, target_img, weights_matrix)


def beam_search(target_img, n_brushstrokes=NUM_OF_BRUSHSTROKES, beam_width=BEAM_WIDTH,
    ...
    for _ in tqdm(range(iterations)):
        ...
        for node in beam:
            ...
            for new_tile in new_tiles:
                ...
                new_canvas = Node(new_rectangle_list, target_img, init_node.weights_matrix, parent=node)
...
if __name__ == '__main__':
    ...
    final_node = beam_search(target_img=target_img, n_brushstrokes=NUM_OF_BRUSHSTROKES, beam_width=BEAM_WIDTH,
                             n_candidates=RANDOM_TILES_PER_ROUND, iterations=ITER_NUM)
